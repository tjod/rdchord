...
Grant Select On tpsa To Public;

COPY tpsa (atom_psa, smarts, description) FROM stdin;
23.79	[N0;H0;D1;v3]	N#
...
COMMENT ON TABLE tpsa IS 'topological polar surface area
...
COMMENT ON COLUMN tpsa.atom_psa IS 'parameterized partial surface area of atom fragment';
COMMENT ON COLUMN tpsa.smarts IS 'fragment smarts';
COMMENT ON COLUMN tpsa.description IS 'pseudo-smarts description and more';


Create OR Replace FUNCTION tpsa(Bytea)
 Returns numeric AS '
...
Select sum(atom_psa*rdkit.count_matches($1,smarts))
   from tpsa;
' LANGUAGE sql IMMUTABLE;
Comment on FUNCTION tpsa(Bytea)
