DROP TABLE If Exists public166keys;

CREATE TABLE public166keys (
    smarts Text NOT NULL,
    "bit" integer NOT NULL,
    count integer,
    description Text
);
Grant Select On public166keys To Public;

--[!0*]	1	ISOTOPE	0
COPY public166keys (smarts, "bit", description, count) FROM stdin;
...
COMMENT ON TABLE public166keys IS '166 public keys from MDL, courtesy of Jeremy Yang.';

Create or Replace FUNCTION public166keys(Bytea) Returns Integer[] As $EOSQL$
 Select array_agg(bit)
  From public166keys Where rdkit.count_matches($1,smarts) > count;
$EOSQL$ Language SQL Immutable;
Comment on Function public166keys(Bytea)
 Is 'public 166 fragment keys of input binary molecule';

Create or Replace FUNCTION public166keys(Text) Returns Integer[] As $EOSQL$
 Select public166keys(rdkit.smiles_to_rdmol($1));
$EOSQL$ Language SQL Immutable;
Comment on Function public166keys(Bytea)
 Is 'public 166 fragment keys of input binary molecule';

Create or Replace FUNCTION public166fp(Bytea) Returns Bit(166) As $EOSQL$
 Select orsum(bit_set(0::Bit(166), bit))
  From public166keys Where rdkit.count_matches($1,smarts) > count;
$EOSQL$ Language SQL Immutable;
Comment on Function public166fp(Bytea)
 Is 'public 166 fragment keys of input binary molecule';

/*
 Select smiles,description from public166keys,
  (Select smiles from nci.structure limit 2) atmp
 Where isbit_set( public166keys(smiles), bit )
  Order By smiles
*/
